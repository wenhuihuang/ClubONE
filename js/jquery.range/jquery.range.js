(function($, window, document, undefined) {
	...
	jRange.prototype = {
		...
		showPointerValue: function(pointer, position, animate) {
			...
			if(parseInt(text) == this.options.to){
				var width = label.html(text+this.options.unit+"+").width(),
			...
			//後加的 加至字
			var pointerLabel = $('.pointer-label')
			var w = parseInt($(pointerLabel[1]).css('left'))-parseInt($(pointerLabel[0]).css('left'));
			$('.to-text').css({'left':w/2+parseInt($(pointerLabel[0]).css('left'))+'px'});
		},
